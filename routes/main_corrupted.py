...
def stock_movements():
    ...
    try:
        ...
        sistem_seri = request.args.get('sistem_seri', '')  # Get sistem_seri from query parameters
        ...
        # Toplam ürün sayısı
        total_query = query.replace("SELECT * FROM stocks WHERE 1=1", "SELECT COUNT(*) FROM stocks WHERE 1=1")
        total = db.execute(total_query, params).fetchone()[0]

        # Ürünleri çekme
        query += " LIMIT ? OFFSET ?"
        params.extend([per_page, offset])
        stocks = db.execute(query, params).fetchall()

        # Rezervasyon bilgilerini çekme
        reservations = db.execute("SELECT * FROM reservations").fetchall()

        # Ürünlerle rezervasyonları birleştirme
        stocks_with_reservations = []
        for stock in stocks:
            stock_reservations = [r for r in reservations if r['product_id'] == stock['id']]
            stocks_with_reservations.append({
                **stock,
        ...
        return render_template('stock_list.html',
                             ...
                             location=location,
                             color=color,
                             sistem_seri=sistem_seri,  # Pass sistem_seri to template
                             sort_by=sort_by,
                             sort_order=sort_order)
